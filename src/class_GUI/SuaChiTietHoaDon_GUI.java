...
public class SuaChiTietHoaDon_GUI extends JFrame implements WindowListener, ListSelectionListener {
	...
	private boolean xoaMotChiTietHoaDon(int index) {
	...
	private float tinhTongTienTraLaiChoKhach(float thanhTien) {
		String tongTienTraLaiHienTai = txtTongTienTraLaiChoKhach.getText();
		float tongTienTraLaiChoKhach = Float.parseFloat(loaiBoDinhDangTien(tongTienTraLaiHienTai));
		tongTienTraLaiChoKhach += thanhTien;
		if (radCo.isSelected()) {
			tongTienTraLaiChoKhach = tongTienTraLaiChoKhach * (1 - hoaDon.getMienGiam());
		}
		return tongTienTraLaiChoKhach;
	}
	 ...
	 * Create the frame.
	 */
	public SuaChiTietHoaDon_GUI(HoaDon hd, NhanVien nv, String tenChucNangYeuCauBanDau) {
		hoaDon = hd;
		emp = nv;
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage("data\\icons\\more-details-16.png"));
		// menuBar
		...
		btnXoa.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (row != -1) {
					...
					int confirm = JOptionPane.showConfirmDialog(null, "Bạn có chắc chắn muốn xóa dòng sản phẩm này hay không?", "Chú ý",
					...
					if (confirm == JOptionPane.YES_OPTION) {
						...
						if (count_row > 1) {
							...
							String string_thanhTien = String.valueOf(tableChiTietHoaDon.getValueAt(row, 9));
							float float_ThanhTien = Float.parseFloat(loaiBoDinhDangTien(string_thanhTien));
							ChiTietHoaDon_DAO cthd_dao = new ChiTietHoaDon_DAO();
							if (xoaMotChiTietHoaDon(row)) {
								if (cthd_dao.xoaMotChiTietHoaDon(maHoaDon, maThuoc)) {
									Thuoc_DAO thuoc_dao = new Thuoc_DAO();
									Thuoc thuoc = thuoc_dao.layThuoc(maThuoc);
									String soLuongBan = String.valueOf(tableChiTietHoaDon.getValueAt(row, 8));
									int soLuongTon = thuoc.getSoLuongTon() + Integer.parseInt(soLuongBan);
									if (thuoc_dao.capNhatSoLuongTonCuaThuoc(soLuongTon, maThuoc)) {
										txtTongTienThanhToan.setText(fmt.format(tinhTongThanhTien()));
										...
										txtTongTienTraLaiChoKhach.setText(fmt.format(tinhTongTienTraLaiChoKhach(float_ThanhTien)));
									} else {
										Toolkit.getDefaultToolkit().beep();
										JOptionPane.showMessageDialog(null, "Lỗi khi cập nhật lại số lượng tồn của sản phẩm này trong CSDL", "Lỗi kết nối CSDL", JOptionPane.ERROR_MESSAGE);
									}
								} else {
									Toolkit.getDefaultToolkit().beep();
									JOptionPane.showMessageDialog(null, "Lỗi khi xóa dòng sản phẩm này trong CSDL", "Lỗi kết nối CSDL", JOptionPane.ERROR_MESSAGE);
								}
							}
						} else {
							Toolkit.getDefaultToolkit().beep();
							if (JOptionPane.showConfirmDialog(null,
									"Hóa đơn không thể có dưới 1 sản phẩm. Bạn có muốn xóa hóa đơn này hay không?",
									"Cảnh báo", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
								XoaThongTinHoaDon_GUI xoaHoaDon = new XoaThongTinHoaDon_GUI(hoaDon, emp, tenChucNangYeuCauBanDau);
								xoaHoaDon.setVisible(true);
								setVisible(false);
							}
						}
					}
		...
		JButton btnXong = new JButton("Xong");
		btnXong.setBackground(Color.WHITE);
		btnXong.setIcon(new ImageIcon("data\\icons\\check-16.png"));
		btnXong.setFocusable(false);
		panel.add(btnXong);
		...
		txtTongTienTraLaiChoKhach = new JTextField();
		txtTongTienTraLaiChoKhach.setText("0");
