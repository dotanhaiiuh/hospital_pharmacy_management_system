...
public class SuaThongTinKhacHang_GUI extends JFrame implements WindowListener, ListSelectionListener {
	 ...
	 * Create the frame.
	 */
	public SuaThongTinKhacHang_GUI(KhachHang khachHangCanChinhSua, NhanVien nhanVienThucHienChinhSua,
			String tenFormTruocDo) {
		emp = nhanVienThucHienChinhSua;
		khachHang = khachHangCanChinhSua;
		getContentPane().setBackground(Color.WHITE);
		setBackground(Color.WHITE);
		setIconImage(Toolkit.getDefaultToolkit().getImage("data\\icons\\edit-customer.png"));
		// menuBar
			...
			public void actionPerformed(ActionEvent e) {
				if (validData()) {
					...
					int confirm = JOptionPane.showConfirmDialog(null,
							"Bạn có chắc chắn muốn sửa thông tin khách hàng này hay không?", "Chú ý",
							JOptionPane.YES_NO_OPTION);
					if (confirm == JOptionPane.YES_OPTION) {
						KhachHang khachHang = taoKhachHang();
						KhachHang_DAO kh_dao = new KhachHang_DAO();
						if (kh_dao.capNhatThongTinKhachHang(khachHang, khachHangCanChinhSua.getMaKH())) {
							Toolkit.getDefaultToolkit().beep();
							JOptionPane.showMessageDialog(null, "Sửa thông tin khách hàng thành công");
							if (tenFormTruocDo.equalsIgnoreCase("Tạo hóa đơn")) {
								TaoHoaDon_GUI taoHoaDon = new TaoHoaDon_GUI(emp);
								taoHoaDon.setVisible(true);
								setVisible(false);
							} else if (tenFormTruocDo.equalsIgnoreCase("Thêm khách hàng")) {
								ThemKhacHang_GUI themKhacHang = new ThemKhacHang_GUI(emp);
								themKhacHang.setVisible(true);
								setVisible(false);
							} else if (tenFormTruocDo.equalsIgnoreCase("Tìm thông tin khách hàng")) {
								TimThongTinKhachHang_GUI timThongTinKhachHang = new TimThongTinKhachHang_GUI(emp,
										getTitle());
							...
							JOptionPane.showMessageDialog(null,
									"Thêm khách hàng không thành công do trùng mã khách hàng", "Lỗi ràng buộc CSDL",
		...
		lblTitle.setIcon(new ImageIcon("data\\icons\\edit-customer-title.png"));
		...
		datePickerNgaySinh.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				LocalDate date_NgayDangKy = LocalDate.parse(ngayDangKy, dtf);
				if (date_NgaySinh.isEqual(date_NgayDangKy) || date_NgaySinh.isAfter(date_NgayDangKy)) {
					Toolkit.getDefaultToolkit().beep();
					JOptionPane.showMessageDialog(datePickerNgaySinh,
							"Ngày sinh không được lớn hơn hoặc bằng ngày đăng ký", "Cảnh báo",
							JOptionPane.WARNING_MESSAGE);
					datePickerNgaySinh.getJFormattedTextField().setText("");
				} else if (date_NgayDangKy.getYear() - date_NgaySinh.getYear() < 14) {
					Toolkit.getDefaultToolkit().beep();
