...
public class SuaThongTinNhaCungCap_GUI extends JFrame implements WindowListener, ListSelectionListener {
	...
	private void fillForm(int row) {
		if (row != -1) {
			...
			String tenNCC = String.valueOf(tableNhaCungCap.getValueAt(row, 1));
			txtTenNhaCungCap.setText(tenNCC);
			String email = String.valueOf(tableNhaCungCap.getValueAt(row, 2));
			txtEmail.setText(email);
			String soDienThoai = String.valueOf(tableNhaCungCap.getValueAt(row, 3));
			txtSoDienThoai.setText(soDienThoai);
			String diaChi = String.valueOf(tableNhaCungCap.getValueAt(row, 4));
	 ...
	 * Create the frame.
	 */
	public SuaThongTinNhaCungCap_GUI(NhaCungCap nhaCungCap, NhanVien nv, String tenFormTruocDo) {
		emp = nv;
		getContentPane().setBackground(Color.WHITE);
		setBackground(Color.WHITE);
		setIconImage(Toolkit.getDefaultToolkit().getImage("data\\icons\\icons8-edit-suppier-24.png"));
		// menuBar
				...
				if (validData()) {
					...
					if (confirm == JOptionPane.YES_OPTION)
					{
						...
						if (ncc_dao.capNhatThongTinNhaCungCap(nhaCungCap)) {
							...
							Toolkit.getDefaultToolkit().beep();
							JOptionPane.showMessageDialog(null, "Lỗi khi sửa thông tin nhà cung cấp trong CSDL", "Lỗi kết nối CSDL",
		...
		lblTitle.setIcon(new ImageIcon("data\\icons\\edit-supplier-title.png"));
		lblTitle.setBounds(381, 10, 850, 89);
