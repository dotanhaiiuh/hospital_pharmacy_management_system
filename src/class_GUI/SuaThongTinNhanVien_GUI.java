...
public class SuaThongTinNhanVien_GUI extends JFrame implements WindowListener, ListSelectionListener {
	...
	private boolean validData() {
		...
		if (cbTinhTrang.getSelectedIndex() == -1) {
			Toolkit.getDefaultToolkit().beep();
			JOptionPane.showMessageDialog(cbTinhTrang, "Bạn phải chọn tình trạng làm việc của nhân viên", "Cảnh báo",
					JOptionPane.WARNING_MESSAGE);
		...
		if (cbTrangThai.getSelectedIndex() == -1) {
			Toolkit.getDefaultToolkit().beep();
			JOptionPane.showMessageDialog(cbTrangThai, "Bạn phải chọn trạng thái tài khoản của nhân viên", "Cảnh báo",
					JOptionPane.WARNING_MESSAGE);
	...
	public SuaThongTinNhanVien_GUI(NhanVien nhanVienCanChinhSua, NhanVien nhanVienThucHienChinhSua, String tenFormTruocDo) {
		...
		btnLuu.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (validData()) {
					if (!nhanVien.getChucVu().equalsIgnoreCase("Quản lý nhà thuốc") || nhanVien.getMaNV().equalsIgnoreCase(emp.getMaNV())) {
						Toolkit.getDefaultToolkit().beep();
						int confirm = JOptionPane.showConfirmDialog(null,
								"Bạn có chắc chắn muốn sửa thông tin nhân viên này hay không?", "Chú ý",
						...
						if (confirm == JOptionPane.YES_OPTION)
						{
							...
							if (nv_dao.capNhatThongTinNhanVien(nv)) {
								...
								if (!tk_dao.capNhatTrangThai(trangThai, nhanVien.getMaNV())) {
									Toolkit.getDefaultToolkit().beep();
									JOptionPane.showMessageDialog(null, "Lỗi khi cập nhật lại trạng thái tài khoản nhân viên vào CSDL", "Lỗi kết nối CSDL", JOptionPane.ERROR_MESSAGE);
								}
								String matKhau = txtPassword.getText().trim();
								if (!matKhau.equalsIgnoreCase("")) {
									if (!tk_dao.capNhatMatKhau(matKhau, nhanVien.getMaNV())) {
										Toolkit.getDefaultToolkit().beep();
										JOptionPane.showMessageDialog(null, "Lỗi khi cập nhật lại mật khẩu nhân viên vào CSDL", "Lỗi kết nối CSDL", JOptionPane.ERROR_MESSAGE);
									}
								}
								if (file != null) {
									...
									if (!tk_dao.capNhatHinhAnh(hinhAnh, nhanVien.getMaNV())) {
										Toolkit.getDefaultToolkit().beep();
										JOptionPane.showMessageDialog(null, "Lỗi khi cập nhật lại ảnh nhân viên vào CSDL", "Lỗi kết nối CSDL", JOptionPane.ERROR_MESSAGE);
									}
								}
								
								if (tenFormTruocDo.equalsIgnoreCase("Tìm thông tin nhân viên")) {
									TimThongTinNhanVien_GUI timThongTinNhanVien = new TimThongTinNhanVien_GUI(emp, getTitle());
									timThongTinNhanVien.setVisible(true);
									setVisible(false);
								}
								else if(tenFormTruocDo.equalsIgnoreCase("Thêm nhân viên"))
								{
								...
								Toolkit.getDefaultToolkit().beep();
								JOptionPane.showMessageDialog(null, "Lỗi khi cập nhật thông tin nhân viên vào CSDL", "Lỗi kết nối CSDL", JOptionPane.ERROR_MESSAGE);
							}
						}
					}
					else
					{
						Toolkit.getDefaultToolkit().beep();
						JOptionPane.showMessageDialog(null, "Bạn không có đủ quyền để cập nhật thông tin của nhân viên này", "Lỗi phân quyền", JOptionPane.ERROR_MESSAGE);
					}
				}
			}
		});
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!ngaySinh.equalsIgnoreCase("")) {
					...
					if (date_NgayVaoLam.isAfter(LocalDate.now())) {
						Toolkit.getDefaultToolkit().beep();
						JOptionPane.showMessageDialog(datePickerNgayVaoLam, "Ngày vào làm không thể sau ngày hiện tại",
						...
						if (date_NgayVaoLam.isBefore(date_NgaySinh)) {
							...
							JOptionPane.showMessageDialog(datePickerNgayVaoLam,
									"Ngày vào làm không thể trước ngày sinh", "Cảnh báo", JOptionPane.WARNING_MESSAGE);
							datePickerNgayVaoLam.getJFormattedTextField().setText("");
						} else {
							if (date_NgayVaoLam.getYear() - date_NgaySinh.getYear() < 18) {
								Toolkit.getDefaultToolkit().beep();
								JOptionPane.showMessageDialog(datePickerNgayVaoLam, "Nhân viên chưa đủ tuổi để đi làm",
										"Cảnh báo", JOptionPane.WARNING_MESSAGE);
								datePickerNgayVaoLam.getJFormattedTextField().setText("");
							} else {
								phatSinhMaNhanVien();
