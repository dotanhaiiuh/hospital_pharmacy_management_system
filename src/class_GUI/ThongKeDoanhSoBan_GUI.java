...
public class ThongKeDoanhSoBan_GUI extends JFrame implements WindowListener {
	...
	private String[] layDanhSachCacNam() {
		...
		ArrayList<String> danhSachCacNam = hoadon_dao.layCacNamLapHoaDon();
	...
	private static JFreeChart createBarChart(CategoryDataset dataset, int nam) {
		JFreeChart barChart = ChartFactory.createBarChart3D(
				"BIỂU ĐỒ THỐNG KÊ DOANH SỐ BÁN HÀNG QUA CÁC THÁNG TRONG NĂM " + nam, "Tháng", "Số lượng", dataset,
	...
	private static CategoryDataset taoDuLieuBieuDoTheoNam(int nam) {
		...
		for (int i = 0; i < 12; i++) {
			int thang = i+1;
			float doanhSoBan = hoadon_dao.layDoanhSoBanTheoThang(thang, nam);
			dataset.addValue(doanhSoBan, "Tháng", "Tháng " + thang);
		}
		return dataset;
	}
	...
	private static void taoBieuDoThongKeDangCot(CategoryDataset duLieuChoBieuDo, int nam) {
		...
		lblTitle.setIcon(new ImageIcon("data\\icons\\statistics-sales.png"));
		lblTitle.setBounds(426, 23, 668, 82);
	 ...
	 * Create the frame.
	 */
	public ThongKeDoanhSoBan_GUI(NhanVien nv) {
		emp = nv;
		...
		JLabel lblNam = new JLabel("Chọn năm muốn xem doanh số bán hàng:");
