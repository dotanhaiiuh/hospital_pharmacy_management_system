...
public class TimKiemThuoc_GUI extends JFrame implements ActionListener, WindowListener, ListSelectionListener {
	...
	private ArrayList<Thuoc> danhSachTatCaThuoc;
	...
	private void capNhatDuLieuTrenComboBoxMaThuoc() {
		...
		if (danhSachTatCaThuoc.size() > 0) {
			ArrayList<String> danhSachThuoc = new ArrayList<String>();
			for (Thuoc thuoc : danhSachTatCaThuoc) {
	...
	private void fillForm(int row) {
		if (row != -1) {
			...
			ArrayList<Thuoc> dsThuoc = danhSachTatCaThuoc;
			if (dsThuoc.contains(thuoc)) {
				thuoc = dsThuoc.get(danhSachTatCaThuoc.indexOf(thuoc));
				cbMaThuoc.removeAllItems();
	 ...
	 * Create the frame.
	 */
	public TimKiemThuoc_GUI(NhanVien e) {
		emp = e;
		...
		lblTitle.setIcon(new ImageIcon("data\\icons\\search-medicine-title.png"));
		lblTitle.setBounds(509, 10, 509, 93);
		...
		danhSachTatCaThuoc = thuoc_dao.layTatCaThuoc();
		if (danhSachTatCaThuoc != null) {
			capNhatDuLieuTrenBang_Init(danhSachTatCaThuoc, soPhanTuMuonHienThi);
		} else {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj.equals(cbMaThuoc)) {
			...
			capNhatDuLieuTrenBang_Search(danhSachTatCaThuoc, soPhanTuMuonHienThi);
		} else if (obj.equals(btnNextPage)) {
			tableThuoc.clearSelection();
			capNhatDuLieuTrenBang_Next(danhSachTatCaThuoc, soPhanTuMuonHienThi);
		} else if (obj.equals(btnPreviousPage)) {
			tableThuoc.clearSelection();
			capNhatDuLieuTrenBang_Previous(danhSachTatCaThuoc, soPhanTuMuonHienThi);
		} else if (obj.equals(btnLastPage)) {
			tableThuoc.clearSelection();
			capNhatDuLieuTrenBang_Last(danhSachTatCaThuoc, soPhanTuMuonHienThi);
