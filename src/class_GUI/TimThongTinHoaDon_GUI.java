...
public class TimThongTinHoaDon_GUI extends JFrame implements ActionListener, WindowListener, ListSelectionListener {
	...
	private HoaDon taoHoaDon() {
		...
		if (dsHoaDon.contains(hoaDon)) {
			...
			KhachHang khachHang = khachhang_dao.layKhachHang(maKhachHang);
			String maNhanVien = hoaDon.getNhanVien().getMaNV();
			NhanVien_DAO nhanvien_dao = new NhanVien_DAO();
			NhanVien nhanVien = nhanvien_dao.layNhanVien(maNhanVien);
			Date ngayLapHoaDon = hoaDon.getNgayLapHoaDon();
			String theBaoHiem = hoaDon.getTheBaoHiem();
			float mienGiam = hoaDon.getMienGiam();
			return new HoaDon(maHoaDon, khachHang, nhanVien, ngayLapHoaDon, theBaoHiem, mienGiam);
		} else {
			return null;
		}
	}
	 ...
	 * Create the frame.
	 */
	public TimThongTinHoaDon_GUI(NhanVien e, String tenChucNangDaChon) {
		emp = e;
		...
		lblTitle.setIcon(new ImageIcon("data\\icons\\search-purchase-title.png"));
		lblTitle.setBounds(423, 10, 697, 93);
		...
		cbMaKhachHang.setSelectedItem("");
		cbMaKhachHang.setBounds(801, 60, 249, 29);
		...
		tableHoaDon.setToolTipText("Nhấn đúp chuột vào dòng bất kỳ để xem chi tiết hóa đơn");
		tableHoaDon.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2) {
					if (tenChucNangCanThucHienKhiTimKiem.equalsIgnoreCase("Sửa thông tin hóa đơn")) {
						SuaChiTietHoaDon_GUI suaChiTietHoaDon = new SuaChiTietHoaDon_GUI(taoHoaDon(), emp,
								tenChucNangCanThucHienKhiTimKiem);
						suaChiTietHoaDon.setVisible(true);
					} else if (tenChucNangCanThucHienKhiTimKiem.equalsIgnoreCase("Xóa thông tin hóa đơn")) {
						XoaThongTinHoaDon_GUI xoaHoaDon = new XoaThongTinHoaDon_GUI(taoHoaDon(), emp,
								tenChucNangCanThucHienKhiTimKiem);
		...
		setTitle("Tìm thông tin hóa đơn");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj.equals(cbMaHoaDon)) {
			...
			if (row != -1) {
				if (tenChucNangCanThucHienKhiTimKiem.equalsIgnoreCase("Sửa thông tin hóa đơn")) {
					SuaChiTietHoaDon_GUI xemChiTietHoaDon = new SuaChiTietHoaDon_GUI(taoHoaDon(), emp,
							tenChucNangCanThucHienKhiTimKiem);
					xemChiTietHoaDon.setVisible(true);
				} else if (tenChucNangCanThucHienKhiTimKiem.equalsIgnoreCase("Xóa thông tin hóa đơn")) {
					XoaThongTinHoaDon_GUI xoaHoaDon = new XoaThongTinHoaDon_GUI(taoHoaDon(), emp,
							tenChucNangCanThucHienKhiTimKiem);
					xoaHoaDon.setVisible(true);
				...
				Toolkit.getDefaultToolkit().beep();
				JOptionPane.showMessageDialog(null, "Bạn cần chọn hóa đơn muốn xem chi tiết", "Cảnh báo",
